...
const App: React.FC = () => {
  ...
  const [pendingAction, setPendingAction] = useState<'VIDEO' | 'CSV' | null>(null);

  // -- Hooks for Logic Extraction --
  const { 
    points, setPoints, 
    activeObjectId, setActiveObjectId, 
    numObjects, setNumObjects, 
    isDirty,
    addPoint, deletePoint, 
    resetProject, markAsClean,
    restoreSession 
  ...
  // Attempt to restore session on mount (persistence)
  useEffect(() => {
    restoreSession();
  }, [restoreSession]);
  ...
  const visiblePoints = useMemo(() => {
    return points.filter(p => isTrackingFrame(p.timestamp, videoFps, settings.samplingRateNum, settings.samplingRateDen));
  ...
  const currentFrame = useMemo(() => getFrameIndex(currentTime, videoFps), [currentTime, videoFps]);
  const isFrameAligned = !isPlaying && isTrackingFrame(currentTime, videoFps, settings.samplingRateNum, settings.samplingRateDen);
  ...
  const handleStatusDimensionsChange = useCallback((dims: { width: number, height: number }) => {
  ...
  const handleWidgetDrag = useCallback((x: number, y: number) => {
  ...
  useEffect(() => {
      if (widgetPosition) {
          const isOverlapping = checkObstruction(widgetPosition.x, widgetPosition.y);
      ...
      if (window.showSaveFilePicker) {
        try {
          ...
          if (err.name === 'SecurityError' || (err.message && err.message.includes('Cross origin'))) {
             console.warn('File System Access API restricted. Falling back to download.');
          } else {
    ...
    if (file) {
      ...
      try {
        ...
        if (!isNaN(parsedFps) && parsedFps > 0) {
          ...
          console.warn(`Could not parse FPS from "${fpsStr}", defaulting to 30.`);
          setVideoFps(30);
        }
      } catch (err) {
  ...
  const handleCSVImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (evt) => {
        ...
        setPoints(newPoints);
        const maxId = newPoints.reduce((acc, p) => Math.max(acc, p.objectId), 1);
        setNumObjects(maxId);
        setActiveObjectId(1);
        markAsClean();
      };
  ...
  const deleteCurrent = useCallback(() => {
    ...
    if (isTrackingFrame(currentTime, videoFps, settings.samplingRateNum, settings.samplingRateDen)) {
          ...
          <div className="flex flex-col space-y-8">
             ...
             <div className="flex flex-row items-center gap-6">
                 ...
                 <div className="relative flex-shrink-0 inline-flex items-center justify-center">
                    <Monitor size={80} className="text-white drop-shadow-lg" strokeWidth={1.5} />
                 ...
                 <div className="space-y-1">
                   ...
                   <p className="text-lg text-gray-400 font-light leading-snug">
                     Load a video to record the locations of multiple objects at a specified rate.
                   </p>
                 </div>
             </div>
             ...
             <label className="block w-full max-w-sm ml-2">
                ...
                >
                  <Upload className="mr-3 text-gray-400 group-hover:text-active group-hover:scale-110 transition-all" size={22} />
          ...
          <div className="bg-gray-900 border border-gray-750 rounded-3xl shadow-2xl overflow-hidden">
             ...
             <div className="p-8 border-b border-gray-750 bg-gray-850">
                <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="text-right">
                         <span className="text-gray-200 text-lg font-bold block">Log Location</span>
                         <span className="text-base text-gray-500">Record the active object at the current time</span>
                    </div>
                </div>
             </div>
            ...
            <div className="p-8 space-y-8 bg-gray-900">
                ...
                <div className="grid grid-cols-[1fr_2fr] gap-6 items-center">
                    ...
                    <div>
                        <span className="text-active text-base font-bold block">Active Object</span>
                        <span className="text-gray-500 text-base block mt-1">Current location and trail</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-[1fr_2fr] gap-6 items-center">
                    ...
                    <div>
                        <span className="text-gray-300 text-base font-medium block">Last Known Location</span>
                        <span className="text-gray-500 text-base block mt-1">When object was not tracked within trail duration</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-[1fr_2fr] gap-6 items-center">
                    ...
                    <div>
                        <span className="text-inactive text-base font-medium block">Other Objects</span>
                        <span className="text-gray-500 text-base block mt-1">Click to select as active object</span>
                    </div>
                </div>
            </div>
          </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-950 text-gray-200">
      ...
      <UnsavedChangesDialog 
        ...
        actionDescription={pendingAction === 'CSV' ? 'importing a CSV' : 'loading a new video'}
      />
    </div>
  );
};
