...
type InteractionState = 
  | { type: 'IDLE' }
  | { type: 'PRE_DRAG'; startX: number; startY: number; candidateObj?: { objectId: number, point?: TrackPoint } }
...
export function useCanvasInteraction({
  ...
  const findElementAtScreenPos = useCallback((screenX: number, screenY: number): UIElement | null => {
      ...
      for (let i = uiElements.length - 1; i >= 0; i--) {
          ...
          const dist = Math.sqrt(Math.pow(relX - elScreenX, 2) + Math.pow(relY - elScreenY, 2));
  ...
  const updateDebugState = useCallback((overrides: Partial<InteractionDebugState>) => {
      setDebugState(prev => {
  ...
  const handleLogPoint = (e: React.MouseEvent | React.PointerEvent) => {
      ...
      const isValidFrame = isTrackingFrame(currentTime, videoFps, settings.samplingRateNum, settings.samplingRateDen);
      ...
      if (videoX < 0 || videoX > videoDimensions.w || videoY < 0 || videoY > videoDimensions.h) return;
  ...
  useEffect(() => {
    if (stateRef.current.type === 'IDLE' && cursorPosRef.current) {
        const hitElement = findElementAtScreenPos(cursorPosRef.current.x, cursorPosRef.current.y);
        updateDebugState({ 
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (hitElement) {
        ...
        const isCurrentCircle = hitElement.isCurrent && hitElement.type === 'circle';
        let pointData: TrackPoint | undefined;
        const currentFrameIndex = getFrameIndex(currentTime, videoFps);
        
        if (isCurrentCircle) {
             pointData = points.find(p => p.objectId === hitElement.objectId && getFrameIndex(p.timestamp, videoFps) === currentFrameIndex);
        }
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (stateRef.current.type === 'PANNING') {
        ...
        const proposed = { ...transform, x: transform.x + dx, y: transform.y + dy };
        const constrained = getConstrainedTransform(proposed);
        
        setTransform(constrained);
        
        // Update Ref for next delta
        stateRef.current = { ...panningState, lastX: e.clientX, lastY: e.clientY };
    }
  };
