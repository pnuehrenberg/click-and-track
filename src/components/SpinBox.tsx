...
export const SpinBox: React.FC<SpinBoxProps> = ({
  ...
  return (
    <div
      className={`flex items-stretch bg-gray-800/80 rounded-lg border border-gray-700 overflow-hidden group focus-within:border-active transition-colors shadow-sm ${className}`}
    >
      <input
        type="number"
        value={value}
        onChange={handleChange}
        className="w-full min-w-0 bg-transparent text-center text-gray-200 font-mono text-lg focus:outline-none [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [-moz-appearance:textfield] px-2 py-1"
      />
