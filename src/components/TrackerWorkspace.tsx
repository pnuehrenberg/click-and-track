...
import { MousePointer2, Hand, Move, ZoomIn, HelpCircle, Crosshair } from 'lucide-react';
...
export const TrackerWorkspace: React.FC<Props> = ({
  ...
  const { videoRef, currentTime, videoDimensions, onLoadedMetadata } = useVideoSynchronization({
    videoSrc,
    videoFps,
    isPlaying,
    setIsPlaying,
  ...
  const getInteractionPills = (state: typeof debugState) => {
    ...
    if (state.isCtrl) {
      return [{ 
        ...
        description: <div className="flex items-center gap-2 text-gray-200">
    ...
    pills.push({
        id: 'hint-ctrl',
        actionIcon: <Crosshair size={18} />,
        description: <div className="flex items-center gap-1.5 text-gray-200">
            <Key>Ctrl</Key>
            <PiMouseLeftClickFill size={16} className="text-gray-300" />
        ...
        >
            ...
            <Tooltip content="Keyboard shortcuts" shortcut={['Esc']} side="bottom">
                ...
                >
                    ...
                    <HelpCircle size={20} className="text-active transition-transform group-hover:scale-110" />
                </button>
            </Tooltip>

            {/* Status Pills */}
            {pills.map(pill => (
                <div key={pill.id} className="flex items-stretch h-11 bg-gray-900/80 backdrop-blur border border-gray-700/50 rounded-full shadow-xl transition-all duration-200 pointer-events-none whitespace-nowrap overflow-hidden">
                    {/* Action Icon Section */}
