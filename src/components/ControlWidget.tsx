...
import { Upload, Download, Plus, ChevronLeft, ChevronRight, FileVideo, ArrowLeftToLine, ArrowLeft, ArrowRight, ArrowRightToLine } from 'lucide-react';
...
export const ControlWidget: React.FC<ControlWidgetProps> = ({
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    element.classList.remove('transition-transform', 'duration-300', 'ease-out');
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    widgetRef.current.classList.add('transition-transform', 'duration-300', 'ease-out');
  };
  ...
  const style: React.CSSProperties = (currentX !== undefined && currentY !== undefined)
      ...
      <div className="bg-gray-900/95 backdrop-blur border border-gray-750/50 rounded-xl shadow-2xl overflow-hidden transition-colors">
        ...
        <div className="grid grid-cols-[2fr_1fr_1fr] gap-4 px-6 py-4 bg-gray-950/80 border-b border-gray-800">
          <div className="flex flex-col justify-start">
            <span className="text-base text-gray-500 font-medium mb-1">Time</span>
            <span className={`font-mono text-xl tracking-tight leading-none ${isFrameAligned ? 'text-active' : 'text-gray-200'}`}>
              {formatTime(currentTime)}
            </span>
          </div>
          
          <div className="flex flex-col justify-start">
            <span className="text-base text-gray-500 font-medium mb-1">Frame</span>
            <span className="font-mono text-xl text-gray-200 tracking-tight leading-none">{currentFrame}</span>
          </div>

          <div className="flex flex-col justify-start">
            <span className="text-base text-gray-500 font-medium mb-1">FPS</span>
            <span className="font-mono text-xl text-gray-200 tracking-tight leading-none">{videoFps.toFixed(2)}</span>
          </div>
        </div>

        {/* Divider / State Indicator */}
        <div className={`h-1 w-full transition-colors duration-200 ${isPlaying ? 'bg-gray-800' : (isFrameAligned ? 'bg-active shadow-[0_0_10px_rgba(239,138,98,0.5)]' : 'bg-gray-800')}`} />
        ...
        <div className="p-4 space-y-4 bg-gray-900/40">
          ...
          <div className="grid grid-cols-3 gap-12">
            <div className="col-span-2 flex flex-col">
              <span className="text-sm text-gray-400 font-medium mb-2">Sampling rate (1 / s)</span>
              <div className="flex items-center gap-2">
                  <SpinBox 
                    value={settings.samplingRateNum}
                    onChange={v => setSettings(s => ({...s, samplingRateNum: Math.max(1, v)}))}
                    min={1}
                    className="w-full"
                  />
                  ...
                  <SpinBox 
                    ...
                    onChange={v => setSettings(s => ({...s, samplingRateDen: Math.max(1, v)}))}
                    min={1}
                    className="w-full"
                  />
              </div>
            ...
            <div className="col-span-1 flex flex-col">
              <span className="text-sm text-gray-400 font-medium mb-2">Trail length</span>
              <Tooltip content="Adjust trail" shortcut={['Ctrl', '+/-']}>
                <SpinBox 
                      value={settings.trailLength}
                      onChange={v => setSettings(s => ({...s, trailLength: Math.max(0, v)}))}
                      min={0}
                  />
              </Tooltip>
            </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              <Tooltip content="Previous object" shortcut={['Shift', 'Tab']}>
                <button 
                  {...prevObjProps}
                  tabIndex={-1}
                  onClick={(e) => { if (e.detail === 0) onPrevObject(); }} // Keyboard access via parent group
                  onPointerDown={(e) => e.stopPropagation()}
              ...
              <div className="flex-1 flex items-center justify-center font-mono text-base font-medium text-gray-200 py-1.5">
                Object&nbsp;<span className="text-active text-lg">{activeObjectId}</span>&nbsp;<span className="text-gray-500">/</span>&nbsp;{numObjects}
              </div>
              ...
              <Tooltip content="Next object" shortcut={['Tab']}>
                <button 
                  {...nextObjProps}
                  tabIndex={-1}
                  onClick={(e) => { if (e.detail === 0) onNextObject(); }} // Keyboard access via parent group
                  onPointerDown={(e) => e.stopPropagation()}
            ...
            <Tooltip content="Add new object track" shortcut={['N']}>
              <button 
                onClick={() => setNumObjects(n => { setActiveObjectId(n+1); return n+1; })}
                onPointerDown={(e) => e.stopPropagation()}
              ...
              >
                <Plus size={20} className="text-gray-400 group-hover:text-active group-hover:scale-110 transition-all" />
              </button>
            </Tooltip>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 pt-1">
            ...
            >
              <FileVideo size={18} className="text-gray-400 group-hover:text-active group-hover:scale-110 transition-all" />
            ...
            <button 
              ...
              className="flex flex-col items-center justify-center space-y-1 bg-gray-800 hover:bg-gray-700 border border-gray-750 text-gray-300 hover:text-white text-xs font-medium py-2 rounded-lg cursor-pointer transition group outline-none focus-visible:border-active"
            >
              <Upload size={18} className="text-gray-400 group-hover:text-active group-hover:scale-110 transition-all" />
            ...
            <button 
              ...
              className="flex flex-col items-center justify-center space-y-1 bg-gray-800 hover:bg-gray-700 border border-gray-750 text-gray-300 hover:text-white text-xs font-medium py-2 rounded-lg cursor-pointer transition group outline-none focus-visible:border-active"
            >
              <Download size={18} className="text-gray-400 group-hover:text-active group-hover:scale-110 transition-all" />
